...
temp_distribution_support_groupby as (
  select
    td.date,
    td.department_code,
    sum(td.distribution) distribution
  from
    temp_distribution_support td
  group by
    1,
    2
),
...
temp_expense_comtor as (
  ...
  where upper(d.department_code) = 'VTI.COMTOR' -- VTI.PQM is unit_id = 14 and ob_id = 1
  group by
    1
),
temp_comtor_effort as (
  select
  ...
  from
    bronze.fact_pbi_vti_comtor_effort ef
    left join temp_expense_comtor te on accounting_date = te.date
    left join (
      select
        *
      from temp_man_month
      where type_id = 'MM001' and upper(department_code) = 'VTI.COMTOR'
    ) tmm on ef.accounting_date = tmm.date
),
temp_distribution_comtor as (
  select
    tdq.date,
    tdq.department_code,
    case
  ...
  from (
    ...
    from
      temp_comtor_effort t
    where upper(t.department_code) <> 'VTI.COMTOR'
  ) as tdq
  group by
    1,
    2
...
temp_distribution_comtor_overhead as (
  select
    tmm.date,
    tmm.unit_id,
    tmm.ob_code,
    tmm.department_code,
    sum(td.expense) expense,
    avg(tmm.calendar_mm) calendar_mm,
    avg(tmm.calendar_mm_total) calendar_mm_total,
    avg(tmm.calendar_mm) * sum(td.expense) / avg(tmm.calendar_mm_total) distribution
  from (
    select
      tmm.date,
      tmm.unit_id,
      ...
      tmm.department_code,
      tmm.value calendar_mm,
      SUM(tmm.value) FILTER (
        WHERE tmm.ob_code in ('VN','SnP')
        and upper(tmm.department_code) not in ('VTI.VMS','VTI.DS','VTI.COMTOR','VTI.PQM')
      ) OVER (PARTITION BY tmm.date) calendar_mm_total
    from
      temp_man_month tmm
    where
      tmm.type_id = 'MM001'
  ...
  inner join (
    select
      tdc.date,
      tdc.department_code,
      tdc.distribution_regular expense
    from temp_distribution_comtor tdc
    where tdc.department_code = 'Overhead'
    union all
    select
      tdc.date,
      tdc.department_code,
      tdc.distribution_regular expense
    from temp_distribution_qa tdc
    where tdc.department_code = 'Overhead'
  ) td on tmm.date = td.date
  group by 1, 2, 3, 4
  having upper(tmm.department_code) not in ('VTI.VMS','VTI.DS','VTI.COMTOR','VTI.PQM') and tmm.ob_code in ('VN', 'SnP')
)
select
  *
from
  temp_distribution_comtor_overhead tdco
order by
