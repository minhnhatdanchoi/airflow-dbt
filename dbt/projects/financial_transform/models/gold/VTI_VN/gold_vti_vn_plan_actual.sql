{{ config(
    materialized='table',
    post_hook=[
        "CREATE INDEX IF NOT EXISTS idx_vti_vn_plan_actual_value_type ON {{ this }}(value_type);",
        "CREATE INDEX IF NOT EXISTS idx_vti_vn_plan_actual_expense_type_5_id ON {{ this }}(expense_type_5_id);",
        "CREATE INDEX IF NOT EXISTS idx_vti_vn_plan_actual_source ON {{ this }}(source);",
    ]
) }}
WITH fact_actual AS (
  SELECT
...
fact_snp_detail AS (
  SELECT
    ...
    'actual' source
  FROM
    {{ ref('silver_fact_vti_snp_detail')}} f
  UNION ALL
  SELECT
    f.accounting_date,
    f.financial_report_value,
    ...
    'planning' source
  FROM
    {{ ref('silver_fact_vti_snp_planning_detail')}} f
),
--dim expense
dim_expense AS (
  SELECT
...
er AS (
  ...
  LEFT JOIN dim_department dpt ON UPPER(f.department_code) = UPPER(dpt.department_code)
  WHERE
    UPPER(f.department_code) = 'VSL.COMMON'
),
-- Lợi nhuận
profit AS (
  SELECT
