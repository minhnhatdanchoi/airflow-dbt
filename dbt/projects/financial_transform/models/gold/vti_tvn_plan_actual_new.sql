{{ config(
    materialized='table',
    post_hook=[
        "CREATE INDEX IF NOT EXISTS idx_tvn_new_value_type ON {{ this }}(value_type);",
    ]
) }}
WITH fact_actual AS (
  SELECT 
    ...
    val.expense_code 
  FROM 
    {{ ref('silver_fact_tvn_accounting_ledger')}} val
  UNION ALL 
  SELECT 
    va.accounting_date, 
    ...
    va.expense_code 
  FROM 
    {{ ref('silver_fact_tvn_adjustment')}} va
), 
fact_planning AS (
  SELECT 
    val.accounting_date, 
    ...
    val.expense_code 
  FROM 
    {{ ref('silver_fact_tvn_accounting_ledger_planning')}} val 
),
fact AS (
  SELECT 
    fa.*, 
...
out as(
	SELECT 
	    erc.*, 
	    COALESCE(revenue.value, 0) AS str_value
	FROM erc
	LEFT JOIN revenue 
	    ON date_trunc('month', erc.date) = date_trunc('month', revenue.date) and erc.source = revenue.source
)
SELECT * FROM out
