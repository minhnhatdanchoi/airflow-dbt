{{ config(
    materialized='table',
    post_hook=[
        "CREATE INDEX IF NOT EXISTS idx_er_snp_new_value_type ON {{ this }}(value_type);",
        "CREATE INDEX IF NOT EXISTS idx_er_snp_new_expense_type_5_id ON {{ this }}(expense_type_5_id);"
    ]
) }}
WITH fact_actual AS (
  SELECT
