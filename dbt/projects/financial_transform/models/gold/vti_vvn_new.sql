{{ config(
    materialized='table',
    post_hook=[
        "CREATE INDEX IF NOT EXISTS idx_vvn_value_type ON {{ this }}(value_type);",
        "CREATE INDEX IF NOT EXISTS idx_vvn_expense_type_5_id ON {{ this }}(expense_type_5_id);"
    ]
) }}
with fact_actual as(
  select 
