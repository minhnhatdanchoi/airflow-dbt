...
fact_apac AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_apac_plan_actual")}} f
  WHERE f.value_type IN ('e', 'r')
),
fact_snp AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_snp_plan_actual")}} f
  WHERE f.value_type IN ('e', 'r')
),
fact_group AS (
  SELECT
...
fact_gits AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_gits_plan_actual")}} f
  WHERE f.value_type IN ('e', 'r')
),
fact_edu AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_edu_plan_actual")}} f
  WHERE f.value_type IN ('e', 'r')
),
fact_tvn AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_tvn_plan_actual")}} f
  WHERE f.value_type IN ('e', 'r')
),
fact_cloud AS (
  SELECT
...
fact_vjp AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_vjp_plan_actual")}} f
...
fact_vkr AS (
  SELECT
    ...
    f.source
  FROM
    {{ ref("gold_vti_vkr_plan_actual")}} f
  LEFT JOIN fact_exchange_rate cer ON DATE_TRUNC('month', f.date) = DATE_TRUNC('month', cer.accounting_date)
  	AND f.source = cer.source
