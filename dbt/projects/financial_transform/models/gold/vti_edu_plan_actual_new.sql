{{ config(
    materialized='table',
    post_hook=[
        "CREATE INDEX IF NOT EXISTS idx_er_edu_new_value_type ON {{ this }}(value_type);",
    ]
) }}
WITH fact_actual AS (
  SELECT 
    ...
    val.expense_code 
  FROM 
    {{ref('silver_fact_edu_accounting_ledger')}} val 
), 
fact_planning AS (
  SELECT 
    val.accounting_date, 
    ...
    val.expense_code 
  FROM 
    {{ref('silver_fact_edu_accounting_ledger_planning')}} val 
), 
fact AS (
  SELECT 
    fa.*, 
